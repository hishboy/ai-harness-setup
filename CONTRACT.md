...
2. Runs `/workspace/bootstrap.sh` **ONCE** (first boot). A marker file then skips
   re-running it on later relaunches/reconnects.
3. Loops: runs `/workspace/launch.sh` (which `exec`s the harness), and drops to a
   held `bash` between runs. `launch.sh` runs on **EVERY** launch.
